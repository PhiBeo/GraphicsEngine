...
void SumEngine::GameWorld::LoadLevel(const std::filesystem::path& levelFile, bool isEditor)
{
	...
	uint32_t capacity = static_cast<uint32_t>(doc["Capacity"].GetUint());
	Initialize(capacity);
...
bool SumEngine::GameWorld::IsInEditMode() const
{
	return mIsEditor;
}

bool SumEngine::GameWorld::IsRequestEdit() const
{
...
{
	if (sEditTemplateFile.empty() || mEditGameObject == nullptr) return;
