...
void SumEngine::MeshComponent::Deserialize(const rapidjson::Value& value)
{
	RenderObjectComponent::Deserialize(value);

	ASSERT(value.HasMember("Shape") || !mModel.meshData.empty(), "MeshComponent: either needs shape data or already has data");
	...
	if (value.HasMember("Shape"))
	{
		...
		if (shapeData.HasMember("Type"))
		{
			...
			else if (shapeType == "Plane")
			{
				...
				uint32_t col = static_cast<uint32_t>(shapeData["Columns"].GetInt());
				float spacing = shapeData["Spacing"].GetFloat();
				meshData.mesh = Graphics::MeshBuilder::CreateGroundPlane(rows, col, spacing);
			}
...
void SumEngine::MeshComponent::Serialize(rapidjson::Document& doc, rapidjson::Value& value, const rapidjson::Value& original)
{
...
const Graphics::Model& SumEngine::MeshComponent::GetModel() const
{
	return mModel;
}
