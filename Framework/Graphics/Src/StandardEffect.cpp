...
void StandardEffect::Initialize(const std::filesystem::path& path)
{
    ...
    mBoneTranformBuffer.Initialize(MaxBoneCount * sizeof(Math::Matrix4));
}

void StandardEffect::Terminate()
{
    mBoneTranformBuffer.Terminate();
...
void StandardEffect::Render(const RenderGroup& renderGroup)
{
    ...
    if (settingsData.useSkinning)
    {
        ...
        AnimationUtil::ComputeBoneTransforms(renderGroup.modelId, boneTranforms, renderGroup.animator);
        AnimationUtil::ApplyBoneOffset(renderGroup.modelId, boneTranforms);

        for (auto& transform : boneTranforms)
        {
            transform = Transpose(transform);
        }
        boneTranforms.resize(MaxBoneCount);
        mBoneTranformBuffer.Update(boneTranforms.data());
    }
